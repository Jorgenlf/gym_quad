...
def intersect_light(pos, d):
    '''Returns the distance to the light source if the ray intersects the light source, otherwise returns inf'''
...
@ti.func
def out_dir(n):
    '''Returns a random direction that is close to the normal vector n'''
...
@ti.func
def sdf_normal(p):
    '''Returns the normal vector at point p on the surface of the object'''
    d = 1e-3
...
def next_hit(pos, d):
    ...
    and the color of the object at the hit point'''
