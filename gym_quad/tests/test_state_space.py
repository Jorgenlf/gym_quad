...
def R_wb(x):
    return np.array([
        [np.cos(x[1])*np.cos(x[2]), -np.cos(x[0])*np.sin(x[2])+np.sin(x[0])*np.sin(x[1])*np.cos(x[2]), np.sin(x[0])*np.sin(x[2])+np.cos(x[0])*np.sin(x[1])*np.cos(x[2])],
        [np.cos(x[1])*np.sin(x[2]), np.cos(x[0])*np.cos(x[2])+np.sin(x[0])*np.sin(x[1])*np.sin(x[2]), -np.sin(x[0])*np.cos(x[2])+np.cos(x[0])*np.sin(x[1])*np.sin(x[2])],
        [-np.sin(x[1]), np.sin(x[0])*np.cos(x[1]), np.cos(x[0])*np.cos(x[1])]
    ])
...
def T_wb(x):
    return np.array([
        [1, np.sin(x[0])*np.tan(x[1]), np.cos(x[0])*np.tan(x[1])],
...
class TestStateSpace(unittest.TestCase):
    def run_dynamics(self, init_eta, action):
        ...
        # One timestep, only gravitational force acts
        ...
        for _ in range(1000):
            ...
            a = R_bw(orientation_true) @ np.array([0, 0, g]) + thrust_max * np.array([0, 0, np.sum(action)]) / m - k_lin * velocity_true / m # Gravitation + Thrust - Drag
    ...
    #     """
        # init_eta = np.array([np.pi/4, np.pi/2, -5, 1, np.pi/4 - 1, np.pi/8])
        # init_eta = np.zeros(6)
        # action = np.array([0.2, 0, 0.1, 0])
        # self.run_dynamics(init_eta=init_eta, action=action)
        # for i in np.arange(0, 2.5, 0.5):
